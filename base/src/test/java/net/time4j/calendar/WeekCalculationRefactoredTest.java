...
public class WeekCalculationRefactoredTest {
    ...
    public void getWeekOfYearISO() {
        HijriCalendar base = HijriCalendar.of(HijriAlgorithm.WEST_ISLAMIC_ASTRO, 1438, 1, 11);
        ...
        for (Object[] c : cases) {
            ...
            HijriCalendar cal = base.with(HijriCalendar.DAY_OF_MONTH, day);
            assertThat("Week of year for day " + day, cal.get(element), is(expected));
        }
    }

    @Test
    public void getWeekOfYearIR() {
        HijriCalendar base = HijriCalendar.of(HijriAlgorithm.WEST_ISLAMIC_ASTRO, 1438, 1, 11); // Wednesday
        ...
        for (Object[] c : cases) {
            ...
            int expected = (int) c[1];
            HijriCalendar cal = base.with(HijriCalendar.DAY_OF_MONTH, day);
            assertThat("Week of year (IR model) for day " + day, cal.get(element), is(expected));
        }
    }

    @Test
    public void getWeekOfYearThai() {
        ThaiSolarCalendar base = PlainDate.of(1940, 4, 11).transform(ThaiSolarCalendar.class); // csütörtök
        ...
        for (Object[] c : cases) {
            int day = (int) c[0];
            int expected = (int) c[1];
            ThaiSolarCalendar cal = base.with(ThaiSolarCalendar.DAY_OF_MONTH, day);
            assertThat("Week of year (Thai) for day " + day, cal.get(element), is(expected));
        }
    }
